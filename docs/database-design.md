...
| `file_type` | `VARCHAR(10)` | NOT NULL | `pdf`, `png`, `jpg`, `docx` (validated from the MIME type at upload) |
...
**Design decisions:**
- `storage_path` is a UUID-based key (e.g., `a3f2b1c4.pdf`) generated by the storage layer, not the user's original filename, to prevent path-traversal attacks
